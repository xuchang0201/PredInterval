...
causal_percent <- as.numeric(args[2])
...
beta_name <- paste0(simu_dir, "simulated_data/", "h2_", h2, "_poly_", causal_percent, "_rep_", n_rep, "_beta.txt")
write.table(beta_output, beta_name, row.names=F, col.names=F, quote=F, sep=" ")

#calculate X*Beta by PLINK score
score_name <- paste0(simu_dir, "simulated_data/", "h2_", h2, "_poly_", causal_percent, "_rep_", n_rep, "_X_times_beta")
...
pheno_output <- data.frame(x_beta_prod$IID, scaled_pheno)
colnames(pheno_output) <- c("eid", "y")
pheno_output_name <- paste0(simu_dir, "simulated_data/", "h2_", h2, "_poly_", causal_percent, "_rep_", n_rep, "_cont_pheno_all_samples.txt")
...
pheno_gemma_name <- paste0(simu_dir, "simulated_data/", "h2_", h2, "_poly_", causal_percent, "_rep_", n_rep, "_cont_pheno_gemma_input.txt")

write.table(pheno_training[,2], pheno_gemma_name, row.names=F, col.names=F, quote=F, sep=" ")

##### run linear model regression by GEMMA
pheno_summstat_output <- paste0("h2_", h2, "_poly_", causal_percent, "_rep_", n_rep, "_cont_pheno_summstat")
...
for (k in 1:5){
  ...
  pheno_fold_output_name <- paste0(simu_dir, "simulated_data/", "h2_", h2, "_poly_", causal_percent, "_rep_", n_rep, "_subset", k, "_removed_cont_pheno.txt")
  write.table(pheno_fold[,2], pheno_fold_output_name, row.names=F, col.names=F, quote=F, sep=" ")
  
  subset_removed_summstat_name <- paste0("h2_", h2, "_poly_", causal_percent, "_rep_", n_rep, "_subset", k, "_removed_cont_pheno_summstat")
