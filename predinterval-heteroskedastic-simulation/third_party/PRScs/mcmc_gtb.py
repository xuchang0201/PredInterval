    ...
    for itr in range(1,n_iter+1):
        ...
        for kk in range(n_blk):
            if blk_size[kk] == 0:
                ...
                quad += _scalar(np.dot(np.dot(beta[idx_blk].T, dinvt), beta[idx_blk]))
                mm += blk_size[kk]

        err = max(
            n/2.0*(1.0-2.0*_scalar(np.sum(beta*beta_mrg))+quad),
            n/2.0*_scalar(np.sum(beta**2/psi)),
        )
        sigma = _scalar(1.0/random.gamma((n+p)/2.0, 1.0/err))
        ...
        for jj in range(p):
            psi[jj,0] = gigrnd.gigrnd(
                a-0.5,
                2.0*_scalar(delta[jj,0]),
                _scalar(n*beta[jj,0]**2/sigma),
            )
        psi[psi>1] = 1.0

        if phi_updt == True:
            w = random.gamma(1.0, 1.0/(phi+1.0))
            phi = _scalar(random.gamma(p*b+0.5, 1.0/(_scalar(np.sum(delta))+w)))
    ...
    with open(eff_file, 'w') as ff:
        if write_pst == 'TRUE':
            ...
            for snp, bp, a1, a2, beta in zip(sst_dict['SNP'], sst_dict['BP'], sst_dict['A1'], sst_dict['A2'], beta_est):
                ff.write('%d\t%s\t%d\t%s\t%s\t%.6e\n' % (chrom, snp, bp, a1, a2, _scalar(beta)))
