...
from predinterval_normalization.common import (
    ...
    adjusted_trait_path,
    rebuild_cv_covar_path,
    rebuild_cv_pheno_path,
)


def main() -> int:
    parser = argparse.ArgumentParser(description="Create leave-one-fold-out GWAS phenotype and covariate tables from stage-10 adjusted phenotypes.")
    add_common_generation_args(parser)
    args = parser.parse_args()

    (args.output_root / "11-cv" / "pheno").mkdir(parents=True, exist_ok=True)
    (args.output_root / "11-cv" / "covar").mkdir(parents=True, exist_ok=True)

    tasks = [(gamma_value, seed_index) for gamma_value in args.gamma_values for seed_index in args.seed_indices]
    for gamma_value, seed_index in tqdm(tasks, desc="make cv inputs"):
        ...
        for fold_index in range(N_FOLDS):
            ...
            pheno_path = rebuild_cv_pheno_path(args.output_root, args.h2, args.causal, gamma_value, seed_index, fold_index)
            covar_path = rebuild_cv_covar_path(args.output_root, args.h2, args.causal, gamma_value, seed_index, fold_index)
