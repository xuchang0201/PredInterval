...
def main() -> int:
    parser = argparse.ArgumentParser(description="Merge raw PRS-CS PLINK scores back onto simulated traits to create stage-09 sim_df_PGS files.")
    add_common_generation_args(parser)
    args = parser.parse_args()

    output_dir = args.output_root / "09-sim_df_pgs"
    output_dir.mkdir(parents=True, exist_ok=True)

    tasks = [(gamma_value, seed_index) for gamma_value in args.gamma_values for seed_index in args.seed_indices]
    for gamma_value, seed_index in tqdm(tasks, desc="assemble stage-09 sim_df_PGS"):
        output_path = sim_df_pgs_path(args.output_root, args.h2, args.causal, gamma_value, seed_index)
        ...
        for fold_index in range(N_FOLDS):
            pgs = pd.read_csv(
                append_suffix(
                    raw_score_prefix(args.output_root, args.h2, args.causal, gamma_value, seed_index, fold_index),
                    ".sscore",
                ),
                sep="\t",
            )
