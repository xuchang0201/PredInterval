...
def main() -> int:
    parser = argparse.ArgumentParser(description="Convert raw stage-05 GWAS outputs into PRS-CS summary-stat tables.")
    add_common_generation_args(parser)
    args = parser.parse_args()

    sst_dir = args.output_root / "06-sst"
    ...
    for gamma_value, seed_index, fold_index in tqdm(tasks, desc="prepare raw PRS-CS sst"):
        output_path = raw_sst_path(args.output_root, args.h2, args.causal, gamma_value, seed_index, fold_index)
        if output_path.exists() and not args.overwrite:
            continue

        glm_path = append_suffix(
            gwas_prefix(args.output_root, args.h2, args.causal, gamma_value, seed_index, fold_index),
            ".y.glm.linear",
        )
